...
プロジェクトにライブラリを追加(cargo add)する際、下記の様な出力になった場合、  
コマンドにオフライン指定(--offline)が抜けている可能性があります。
...
キットではインターネットに接続しない環境を想定しており、必要なライブラリ等は事前にダウンロードされています。  
ダウンロード済みのローカルにあるキャッシュを使用する場合、  
下記の様に「--offline」オプションを指定してライブラリの追加を行ってください。  

```
　　cargo add autd3 --offline
```

独自のプログラムを作成する際に外部ライブラリを使用する場合は、  
各ライブラリのローカル環境でのビルド方法を確認してください。

---

### 6.2.2 実行時に「NO AUTD3 device found」
...
```
Error : Link(LinkError { msg: "NO AUTD3 device found" })
```
